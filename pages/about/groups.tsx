    ...
    <div className="min-h-screen bg-[#FCF9F6] text-[#5C2E2E] font-sans selection:bg-[#D4A373] selection:text-white">
      ...
      <main className="pt-28 md:pt-32 lg:pt-40 pb-12 md:pb-20">
        ...
        <section className="container mx-auto px-6 lg:px-12">
          ...
          {/* ✅ 關鍵修正：
            1. md:grid-cols-2 (平板 2 欄)
            2. lg:grid-cols-2 (電腦 2 欄) -> 這樣 10 個小組剛好 5 行，完美平衡！
            3. max-w-5xl mx-auto -> 限制最大寬度，避免 2 欄在超大螢幕被拉太長
          */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 max-w-5xl mx-auto">
            {groupSchedules.map((group) => (
              <div 
                key={group.id}
                className="group relative bg-white border border-[#E6D5D5] p-6 md:p-10 hover:border-[#B95C50] hover:shadow-xl hover:shadow-[#B95C50]/10 transition-all duration-300 flex flex-col items-center text-center"
              >
                {/* 頂部：星期標籤 (懸浮感設計) */}
                <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                   <span className="bg-[#B95C50] text-white text-[10px] md:text-xs font-bold px-4 py-1.5 tracking-widest uppercase shadow-sm rounded-sm">
                     {group.day}
                   </span>
                </div>

                {/* 群組名稱 */}
                <h3 className="text-xl md:text-2xl font-serif font-bold text-[#4A2525] mt-4 mb-3 group-hover:text-[#B95C50] transition-colors">
                  {group.name}
                </h3>
                
                <div className="w-12 h-[1px] bg-[#E6D5D5] my-3 group-hover:w-20 group-hover:bg-[#B95C50] transition-all"></div>

                {/* 時間與地點 (稍微放大文字，因為卡片變寬了) */}
                <div className="space-y-2 text-sm md:text-base text-[#7F5555]">
                ...
                {/* 互動提示 */}
                <div className="mt-6 opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-2 group-hover:translate-y-0 hidden md:block">
                  <span className="text-xs font-bold text-[#B95C50] uppercase tracking-widest border-b border-[#B95C50] cursor-pointer">
                    Join This Group
                  </span>
                </div>
              </div>
