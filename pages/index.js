...
import { bannerQuery, eventQuery } from '../lib/queries'
import Banner from '../components/Banner'
import EventList from '../components/EventList'

export default function Home({ banners, events }) {
  return (
...
export async function getStaticProps() {
  ...
  const newsList = await sanity.client.fetch(`*[_type == "latestNews"] | order(_createdAt desc)[0...6]{ image }`);
  return {
    props: {
      newsList,
    },
...
export async function getStaticProps() {
  const sanity = await import('../lib/sanity');
  const banners = await sanity.client.fetch(`*[_type == "banner"]{ image }`);
  const newsList = await sanity.client.fetch(`*[_type == "latestNews"] | order(_createdAt desc)[0...6]{ image }`);
  const events = await sanity.client.fetch(`*[_type == "event"] | order(date asc)[0...3]{ title, date }`);
  return {
    props: {
      banners,
      newsList,
